...
import { Truck, Save, KeyRound, Plus, RefreshCw, Trash2, X, Copy } from 'lucide-react';
import api from '../../services/api';
import { AdminButton, AdminCard, AdminPageShell, ConfirmModal } from '../../components/admin/ui';
...
export default function AdminShippingApiSettings() {
  ...
  /** Full secret key — only after create or rotate (not stored in list). */
    ...
    try {
      ...
      if (res?.success && res.data?.apiKey) {
        setKeyReveal({
          ...
          headline: 'Partner API key created',
        });
        try {
          await navigator.clipboard.writeText(res.data.apiKey);
          toast.success('API key copied to clipboard');
        } catch {
          toast.success('Copy the key from the dialog — it will not be shown again.', { duration: 6000 });
        }
        setNewClientName('');
    ...
    try {
      ...
      if (res?.success && res.data?.apiKey) {
        setKeyReveal({
          ...
          headline: 'New API key (old key disabled)',
        });
        try {
          await navigator.clipboard.writeText(res.data.apiKey);
          toast.success('New key copied to clipboard');
        } catch {
          toast.success('Copy the new key from the dialog.', { duration: 6000 });
        }
        await load();
      }
    } catch (e) {
  ...
  const confirmDeleteClient = async () => {
    ...
    try {
      setDeleting(true);
      const res = await api.delete(`/shipping/admin/clients/${encodeURIComponent(deleteTarget.id)}`);
      if (res?.success) {
        toast.success('Partner client removed');
  ...
  return (
    ...
    >
      <ConfirmModal
        ...
        title="Remove partner client?"
        description={
          deleteTarget
            ? `Delete "${deleteTarget.name}"? Their x-api-key will stop working immediately. This cannot be undone.`
            : ''
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
      ...
      {keyReveal ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-labelledby="partner-key-modal-title"
        >
          <button
            type="button"
          ...
          />
          <div className="relative w-full max-w-lg rounded-xl border border-slate-200 bg-white p-6 shadow-2xl shadow-slate-900/10">
            <div className="flex items-start justify-between gap-3">
              <h2 id="partner-key-modal-title" className="text-lg font-semibold text-slate-900 pr-8">
                {keyReveal.headline}
              </h2>
              <button
                type="button"
            ...
            {keyReveal.clientName ? (
              <p className="mt-1 text-sm text-slate-600">
                Client: <strong>{keyReveal.clientName}</strong>
              </p>
            ) : null}
            <p className="mt-3 text-sm text-amber-800 bg-amber-50 border border-amber-100 rounded-lg px-3 py-2">
              Save this full key now. It is not stored in plain text — you will not see it again after closing.
            </p>
            <label className="mt-4 block text-xs font-semibold uppercase tracking-wide text-slate-500">Full API key (share with partner)</label>
            <div className="mt-1 flex gap-2">
              <input
                readOnly
                className="admin-field flex-1 font-mono text-xs"
                value={keyReveal.apiKey}
                onFocus={(e) => e.target.select()}
              />
              <AdminButton type="button" variant="secondary" className="shrink-0" onClick={copyRevealedKey}>
                <Copy className="h-4 w-4" />
              </AdminButton>
            </div>
            <div className="mt-4 rounded-lg border border-slate-100 bg-slate-50 p-3 text-sm text-slate-700 space-y-2">
              <p className="font-medium text-slate-800">What to share</p>
              <p className="text-xs text-slate-600">
                <span className="font-semibold">POST</span>{' '}
                <code className="break-all rounded bg-white px-1 py-0.5 text-[11px]">{proxyRatesUrl}</code>
              </p>
              <p className="text-xs text-slate-600">
                Header:{' '}
                <code className="break-all rounded bg-white px-1 py-0.5 text-[11px]">x-api-key: {keyReveal.apiKey}</code>
              </p>
            </div>
            <div className="mt-6 flex justify-end">
              <AdminButton type="button" variant="primary" onClick={() => setKeyReveal(null)}>
                Done
              </AdminButton>
            </div>
          </div>
        ...
        <div className="mx-auto max-w-4xl space-y-6">
          ...
          <AdminCard title="Partner API keys" subtitle="Businesses send this header: x-api-key: zippy_…" action={<KeyRound className="h-5 w-5 text-slate-600" />}>
            ...
            <div className="mt-6 overflow-x-auto">
              <table className="w-full min-w-[640px] text-left text-sm">
                <thead>
                  <tr className="border-b border-slate-200 text-xs uppercase text-slate-500">
                    ...
                    <th className="py-2 pr-2" title="Short id only; the secret is the full zippy_… key from create/rotate">
                      Key id
                    </th>
                      ...
                      <td className="py-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <AdminButton
                            ...
                            aria-label={`Delete ${c.clientName}`}
                            onClick={() => setDeleteTarget({ id: c._id, name: c.clientName })}
                          >
              ...
              {clients.length === 0 ? (
                <p className="mt-3 text-sm text-slate-500">No partner clients yet.</p>
              ) : (
                <p className="mt-3 text-xs text-slate-500">
                  The table only shows a short key id. Share the full <code className="rounded bg-slate-100 px-1">zippy_…</code> secret from the dialog after create or rotate.
                </p>
              )}
            </div>
          </AdminCard>
