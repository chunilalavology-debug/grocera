...
export default function CoAdminDashboard() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const ws = new WebSocket(`ws://${apiUrl.replace('http://', '')}`);
...
const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(error.message || 'Validation failed. Please check all fields.');
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Emit product update event for real-time sync
        window.dispatchEvent(new CustomEvent('productUpdated'));
      } else {
