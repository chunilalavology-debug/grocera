...
### nginx **500** but `curl http://127.0.0.1:5000/` returns HTML (200)

The browser hits **nginx**, not Node directly. A **500** page signed **`nginx/1.24.0`** is generated by **nginx** (or the proxy path), not Express.

**1) Read the real reason (do this first):**

```bash
sudo tail -50 /var/log/nginx/error.log
```

Reload the site, run `tail` again — the new lines usually name the fault (SSL file, `upstream`, `rewrite`, `connect()` failed, etc.).

**2) Broken `Connection` / `Upgrade` headers (very common)**  
If your `location /` sets **`proxy_set_header Connection "upgrade";`** for **all** requests, many stacks return **500** through nginx while **`curl http://127.0.0.1:5000/`** still works. Use the current **`deploy/nginx-grocera.conf`** from the repo (empty **`Connection ""`** for normal proxying), then:

```bash
sudo nginx -t && sudo systemctl reload nginx
```

**3) HTTPS (443) differs from port 80**  
Certbot edits a **separate** `server { listen 443 ssl; ... }` block. If that block does **not** proxy to `127.0.0.1:5000` (or points at PHP/static `root` only), you get errors only in the browser. Compare:

```bash
sudo nginx -T | sed -n '/server_name zippyyy.com/,/^}/p'
```

Every `server_name` that should serve the app needs the same **`proxy_pass http://grocera_node`** (or `http://127.0.0.1:5000`) for **`location /`**.

**4) Duplicate or default sites**  
Another enabled site may claim your domain or `default_server`:

```bash
ls -la /etc/nginx/sites-enabled/
sudo nginx -T 2>/dev/null | grep -E 'server_name|default_server|listen 443'
```

**5) Quick compare from the VPS**

```bash
curl -sS -o /dev/null -w "direct_node %{http_code}\n" http://127.0.0.1:5000/
curl -sS -o /dev/null -w "via_localhost_https %{http_code}\n" -k --resolve zippyyy.com:443:127.0.0.1 https://zippyyy.com/
```

If the first is **200** and the second **500**, fix nginx’s **443** server block (and error log), not Node.
